...
$Log = "logs/maze_new_run.log"
$Err = "logs/maze_new_run.err"
$PidFile = "logs/maze_new_run.pid"

# Không truyền tham số --resume và --start-episode để chạy một experiment hoàn toàn mới từ đầu
$Command = "cmd /c `"$Python run_experiments.py --config experiments.json --headless > $Log 2> $Err`""
...
if ($Result.ReturnValue -eq 0) {
    if ($null -ne $Result.ProcessId) {
        ...
        Write-Host "Tiến trình độc lập chạy mới (New Run) đã khởi tạo thành công. PID: $($Result.ProcessId)" -ForegroundColor Green
    } else {
