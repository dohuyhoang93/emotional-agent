    ...
    for i in range(1, 5):
        try:
            ...
            if "Audit Blocked" in str(e):
                ...
                print(
                    f"   Attempt {i}: Process Rejected (Biz Rule) -> Audit Counted. Error: {e}"
                )
                if i == 4:
                    print(
                        "❌ FAILED: Expected AuditBlockError on 4th attempt but got normal error."
                    )
...
async def run_scenario_d(engine):
    ...
    try:
        ...
        if result["tid"] != main_tid:
            print(
                f"✅ True Parallelism Verified (Different TID: {main_tid} != {result['tid']})"
            )
        else:
