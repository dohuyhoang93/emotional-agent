...
async def run_comprehensive_benchmark():
    ...
    # 1. Setup Data
    small_data = {f"item_{i}": round(random.random(), 4) for i in range(NUM_SMALL_ITEMS)}
    ...
    # Hydrate Heavy Zone
    engine._core.compare_and_swap(engine.state.version, None, {"large_array": large_np}, None)
    ...
    t_merge = await engine.execute("proc_deep_merge_write")
    print(f"Write Duration: {t_merge*1000:.4f} ms")
    # Verify integrity
    assert engine.state.data["domain"]["nested"]["level1"]["level2"]["leaf"] == 100
    assert engine.state.data["domain"]["nested"]["level1"]["level2"]["other"] == "keep me"
    ...
    if HAS_PYDANTIC:
        ...
        print(f"Interoperability Check: ✅ PASSED")
