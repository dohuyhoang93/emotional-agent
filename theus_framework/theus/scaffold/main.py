...
def run_parallel_demo(engine: TheusEngine):
    ...
    # Allocate Heavy Zone
    # (Assuming simple dictionary simulation if alloc method missing in pure python, 
    # but Theus V3 Engine handles it via HeavyZoneAllocator)
    
    # Input Data
    engine.compare_and_swap(engine.state.version, heavy={
        'source_data': input_arr,
        'results_data': np.zeros(size, dtype=np.float64)
    })
    ...
    total_sum_parallel = sum(r['partial_sum'] for r in results)
