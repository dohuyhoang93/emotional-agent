...
impl TheusEngine {
    ...
    fn transaction(slf: Py<TheusEngine>, py: Python, write_timeout_ms: u64) -> PyResult<Transaction> {
        Ok(Transaction {
            ...
            pending_signal: PyDict::new_bound(py).unbind(),
...
impl Transaction {
    ...
    fn new(py: Python, engine: Option<Py<TheusEngine>>, write_timeout_ms: u64) -> PyResult<Self> {
        let engine_obj = match engine {
            Some(e) => e,
            None => {
                // If no engine provided, create a new one (for independent transaction testing)
    ...
    fn outbox(&self) -> OutboxCollector {
        OutboxCollector { buffer: self.pending_outbox.clone() }
    }
    ...
    fn update(&self, py: Python, data: Option<PyObject>, heavy: Option<PyObject>, signal: Option<PyObject>) -> PyResult<()> {
        ...
        if let Some(s) = signal {
             ...
             self.pending_signal.bind(py).call_method1("update", (s_bound,))?;
        }
        Ok(())
    }
    ...
    fn __exit__(
        ...
        // Commit changes...
        // Note: In real implementation, pending_data should be merged with original state update
        // Current logic calls update with pending dicts.
        ...
        // Commit Outbox to Engine
        {
            ...
            // We need Rust reference to TheusEngine struct
