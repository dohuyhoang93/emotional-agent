...
async def verify_leak():
    ...
    try:
        ...
        if current_val == 666:
            print(
                "❌ CRITICAL: 'domain_ctx' allowed in-place mutation of Global State!"
            )
            ...
            if results["domain_ctx_mutation"] == "Allowed (LEAK)":
                print(
                    "⚠️ WARNING: Local mutation was allowed (Reference Leak), even if not propagated or checked here."
                )
