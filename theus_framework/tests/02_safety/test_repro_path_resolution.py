        ...
        if isinstance(domain, dict) or hasattr(domain, 'keys'):
             ...
             if "nested" in str(domain) and "success" in str(domain):
                  ...
                  try:
                      ...
                      val = nested['value']
                      assert val == 'success'
                      print("✅ SUCCESS: Verified nested structure domain.nested.value == 'success'")
                  except Exception as e:
                       # Try attribute access
                       try:
                           val = domain.nested.value
                           assert val == 'success'
                           print("✅ SUCCESS: Verified via attribute domain.nested.value == 'success'")
                       except:
                           print(f"❌ FAILURE: Could not verify nested data. Content: {domain}")
                           sys.exit(1)
                           
    except KeyError:
        print("❌ FAILURE: 'domain' key missing from state.data (despite appearing in keys?)")
        sys.exit(1)
    except Exception as e:
