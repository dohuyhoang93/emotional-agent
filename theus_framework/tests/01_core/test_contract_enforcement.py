...
# --- Setup Context ---
@dataclass
class TestDomain(BaseDomainContext):
    data: Dict[str, Any] = field(default_factory=dict)
    
@dataclass 
class TestContext(BaseSystemContext):
    domain: TestDomain = field(default_factory=TestDomain)
    global_ctx: Any = None
...
class TestContractEnforcement(unittest.TestCase):
    ...
    async def _run_contract_test(self):
        ...
        recipe_camp = AuditRecipe(threshold_max=0, reset_on_success=True, level=AuditLevel.Count)
        ctx = TestContext(domain=TestDomain(data={}))
        engine = theus.engine.TheusEngine(ctx, strict_mode=True, audit_recipe=recipe_camp)
