...
def explicit_failure_rollback(ctx):
    ctx.transaction.update(data={'domain': {'data': {'rollback_target': "FailedState"}}})
    raise RuntimeError("Simulated Crash")
...
async def test_universal_mutation_patterns():
    ...
    assert state['domain']['data']['functional'] == "Success", "Functional update failed"
    assert state['domain']['data']['original'] == "Preserved", "Silent overwrite logic failed"
    ...
    try:
        ...
        assert "read-only" in str(e).lower() or "pure" in str(e).lower() or "permission" in str(e).lower()
    ...
    state = eng.state.data
    assert state['domain']['data']['rollback_target'] == "Safe", f"Rollback failed: {state['domain']['data']['rollback_target']}"
